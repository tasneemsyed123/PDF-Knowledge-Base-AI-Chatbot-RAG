...
ANSWER_SYSTEM_PROMPT = (
    ...
    "Forms and certificates (insurance policies, invoices, ID documents) "
    "often list several similarly-shaped codes/numbers side by side - "
    "registration no., engine no., chassis no., policy no., invoice no., "
    "internal form/section codes printed in a margin. When answering with "
    "a specific number, always name the exact field label it came from "
    "(e.g. \"Vehicle Registration No.: AP39BR0316\", not just the bare "
    "number) so a misread field is visible rather than silently wrong. If "
    "a question uses a colloquial term that could map to more than one "
    "field (e.g. \"bike number\" could mean the registration number or the "
